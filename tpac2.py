...
def brute_force_tsp(distances, cities):
    ...
    for perm in itertools.permutations(range(1, n)):
        ...
        length = calculate_distance(current_route, distances)
...
# --------------------------------------------------------------------
 def nearest_neighbor_tsp(distances, cities, start=0):
    """
    Implémente l’approche du plus proche voisin.
    - start : indice de la ville de départ (par défaut 0 = Alger)
    - distances : matrice NxN
    - cities : liste des noms des villes
    Retourne : (best_route, best_length)
    """
    n = len(cities)
    visited = [False] * n
    best_route = [start]
...
current = start
    for _ in range(n - 1):
        ...
        best_length += nearest_dist
        current = nearest

    # Retour à la ville de départ
    best_length += distances[current][start]
...
print("Nearest Neighbor:", [cities[i] for i in route_nn], "Distance:", length_nn)

route_bf, length_bf = brute_force_tsp(distance_matrix , cities)
print("Brute Force:", [cities[i] for i in route_bf], "Distance:", length_bf)